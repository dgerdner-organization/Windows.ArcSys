...
{
	public partial class ClsAttachment : ClsBaseTable
	{
		...
		public const string TableName = "T_ATTACHMENT";
		...
		{
			return new string[PrimaryKeyCount] { "ATTACHMENT_ID" };
		}

		public const string SelectSQL = @"SELECT * FROM T_ATTACHMENT 
				INNER JOIN T_ISSUE
				ON T_ATTACHMENT.ISSUE_ID=T_ISSUE.ISSUE_ID ";
		...
		protected Int64? _Attachment_ID;
		...
		public Int64? Attachment_ID
		{
			get { return _Attachment_ID; }
			set
			{
				if( _Attachment_ID == value ) return;
		
				_Attachment_ID = value;
				_IsDirty = true;
				NotifyPropertyChanged("Attachment_ID");
			}
		}
		...
		public ClsAttachment() : base() {}
		public ClsAttachment(DataRow dr) : base(dr) {}
		public ClsAttachment(ClsAttachment src) { CopyFrom(src); }
		...
		public void CopyFrom(ClsAttachment src)
		{
			Attachment_ID = src._Attachment_ID;
		...
		{
			ClsAttachment tmp = ClsAttachment.GetUsingKey(Attachment_ID.Value);
		...
		public override int Insert()
		{
			...
			p[3] = Connection.GetParameter
				("@ATTACHMENT_ID", Attachment_ID, ParameterDirection.Output, DbType.Int64, 0);
			...
			const string sql = @"
				INSERT INTO T_ATTACHMENT
					...
					ATTACHMENT)
				VALUES
					(ATTACHMENT_ID_SEQ.NEXTVAL, @ISSUE_ID, @ATTACHMENT_NM,
					@ATTACHMENT)
				RETURNING
					ATTACHMENT_ID, CREATE_USER, CREATE_DT,
			...
			Attachment_ID = ClsConvert.ToInt64Nullable(p[3].Value);
		...
		public override void LoadFromDataReader(DbDataReader dr)
		{
			...
			Attachment_ID = ClsConvert.ToInt64Nullable(dr["ATTACHMENT_ID"]);
		...
		public override void LoadFromDataRow(DataRow dr)
		{
			...
			Attachment_ID = ClsConvert.ToInt64Nullable(dr["ATTACHMENT_ID", v]);
		...
		public override void CopyToDataRow(DataRow dr)
		{
			...
			dr["ATTACHMENT_ID"] = ClsConvert.ToDbObject(Attachment_ID);
		...
		public static ClsAttachment GetUsingKey(Int64 Attachment_ID)
		{
			object[] vals = new object[] {Attachment_ID};
			DataRow dr = Connection.GetDataRowUsingKey
				(TableName, GetPrimaryKeys(), vals);
			return ( dr == null ) ? null : new ClsAttachment(dr);
		}
		public static DataTable GetAll(Int64? Issue_ID)
		{
			...
			if( Issue_ID != null && Issue_ID > 0 )
			{
				...
				sb.Append(" WHERE T_ATTACHMENT.ISSUE_ID=@ISSUE_ID");
			}
