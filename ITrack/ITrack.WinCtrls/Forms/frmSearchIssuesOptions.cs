...
{
	public partial class frmSearchIssuesOptions : frmDialogBase
	{
		#region Fields
		...
		public bool GetSearchCriteria()
		{
			try
			{
				if( FirstLoad )
				{
					ClsITrackConfig.IssueOptions.ApplyIssueOptions(_Options);
				}
		...
		private void SetGUI()
		{
			try
			{
				...
				grpLastAttachDt.Visible = grpLastNoteDt.Visible =
					cmbNewRequirement.Visible = cmbDevAsst.Visible = cmbWIP.Visible =
					cmbRelease.Visible = isDev;
			}
			catch( Exception ex )
			{
		...
		private void OptionsFormLoad()
		{
			try
			{
				...
				if( FirstLoad )
				{
					...
					cmbCategory.Text = _Options.CategoryCds;
					cmbPriority.Text = _Options.PriorityCds;
					cmbNewRequirement.Text = _Options.New_Requirement_Flg;
				}
			}
			catch( Exception ex )
			{
		...
		protected override bool SaveChanges()
		{
			try
			{
				...
				_Options.CategoryCds = cmbCategory.CategoryCDs;

				_Options.Action_Owners = cmbActionOwner.UserLogins;
				...
				_Options.PriorityCds = cmbPriority.PriorityCDs;

				_Options.Emg_Flg = cmbEmg.SelectedCD;
				_Options.Data_Fix_Flg = cmbDataFix.SelectedCD;
				_Options.New_Requirement_Flg = cmbNewRequirement.SelectedCD;
				_Options.Dev_Assist_Flg = cmbDevAsst.SelectedCD;
				_Options.Wip_Flg = cmbWIP.SelectedCD;
				_Options.Release_Flg = cmbRelease.SelectedCD;

				_Options.Create_Dt = grpCreated.CheckedValueRange;
				_Options.Modify_Dt = grpModified.CheckedValueRange;
				_Options.Due_Dt = grpDue.CheckedValueRange;
		...
		private void frmSearchIssuesOptions_Load(object sender, EventArgs e)
		{
			OptionsFormLoad();
		}

		private void frmSearchIssuesOptions_KeyDown(object sender, KeyEventArgs e)
		{
			try
			{
