...
namespace CS2010.Common
{
	public static class ClsExtension
    {
        ...
        public static string TimeZoneCode(this DateTime date)
        {
            StringBuilder result = new StringBuilder();
            foreach (string value in TimeZoneInfo.Local.StandardName.Split(' '))
            {
            ...
            StringBuilder daytimeSaving = new StringBuilder();
            foreach (string value in TimeZoneInfo.Local.DaylightName.Split(' '))
            {
                if (!String.IsNullOrEmpty(value.Trim()))
                {
                    daytimeSaving.Append(char.ToUpper(value[0]));
                }
            }
            return daytimeSaving.Length == 0 ? result.ToString() : result.ToString() + "-" + daytimeSaving.ToString();
        }
        public static string FormatDateTime24HrWithTimeZone(this DateTime date)
        {
            StringBuilder result = new StringBuilder(date.ToString("yyyy-MM-dd-HHmm-"));
            return result.Append(date.TimeZoneCode()).ToString();
        }
