...
{
	public partial class frmConveyanceManager : frmChildBase, ISearchWindow
	{
		#region Fields
		...
		private void frmConveyanceManager_Load(object sender, EventArgs e)
		{
			try
			{
				btnSearch.Focus();
		...
		private void SearchParameters()
		{
			try
			{
				...
				Options.Booking_No = txtBooking.Text.Trim();
		...
		private void StartSearch()
		{
			try
			{
				...
				DataTable dt1 = Options.SearchCargo();
			...
			try
			{
				lock (grdCargo)
				{
					...
					if (string.IsNullOrEmpty(opts))
						...
						cap = "Search for " + opts;
					grpSearch.Text = string.Format("{0} returned {1} Row(s)",
						cap, grdCargo.RecordCount);
				}
			}
			catch (Exception ex)
			{
		...
		private void EditCargoConveyance()
		{
			try
			{
				...
				DataRow[] cnvCargo = dtCargo.Select("conveyance_id = " + cnv.Conveyance_ID);
		...
		private void AddCargo()
		{
			try
			{
				...
				if (dtCargo.HasErrors || vendorMoves.Count != 1)
				{
		...
		private void frmConveyanceManager_FormClosing(object sender, FormClosingEventArgs e)
		{
			try
			{
		...
		private void frmConveyanceManager_KeyDown(object sender, KeyEventArgs e)
		{
			try
			{
