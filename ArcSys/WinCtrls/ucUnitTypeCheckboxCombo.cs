...
{
	/// <summary>ArcSys version of ucCheckedComboBox for unit type codes</summary>
	public partial class ucUnitTypeCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked unit type codes</summary>
		[Browsable(false)]
		public List<string> CheckedUnitTypeCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Unit_Type_Cd"]);
		...
		/// <summary>Gets a comma separated string of unit type codes</summary>
		[Browsable(false)]
		public string UnitTypeCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of unit type descriptions</summary>
		[Browsable(false)]
		public string UnitTypeDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Unit_Type_Dsc"]);
		...
		public ucUnitTypeCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Unit_Type_Cd", "Unit_Type_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more unit type codes are invalid");
							break;
						}
					}
				}
