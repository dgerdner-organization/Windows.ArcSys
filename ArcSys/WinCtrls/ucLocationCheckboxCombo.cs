...
{
	/// <summary>ArcSys version of ucCheckedComboBox for locations</summary>
	public partial class ucLocationCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties
		...
		/// <summary>Gets a list of checked location codes</summary>
		[Browsable(true), DefaultValue(false),
		Description("Set to true to limit the locations to ports")]
		...
		/// <summary>Gets a list of checked location codes</summary>
		[Browsable(false)]
		public List<string> CheckedLocationCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Location_Cd"]);
		...
		/// <summary>Gets a comma separated string of location codes</summary>
		[Browsable(false)]
		public string LocationCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of location descriptions</summary>
		[Browsable(false)]
		public string LocationDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Location_Dsc"]);
		...
		public ucLocationCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Location_Cd", "Location_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more locations are invalid");
							break;
						}
					}
				}
