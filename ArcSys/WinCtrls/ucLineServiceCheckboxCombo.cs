...
{
	/// <summary>ArcSys version of ucCheckedComboBox for Services</summary>
	public partial class ucLineServiceCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

        //private bool _VendorLimited;
        ///// <summary>Gets a list of checked Service codes</summary>
        //[Browsable(true), DefaultValue(false),
        //Description("Set to true to limit the Services to those within a vendors region")]
		...
		/// <summary>Gets a list of checked Service codes</summary>
		[Browsable(false)]
		public List<string> CheckedServiceCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["SERVICE_CD"]);
		...
		/// <summary>Gets a comma separated string of Service codes</summary>
		[Browsable(false)]
		public string ServiceCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of Service descriptions</summary>
		[Browsable(false)]
		public string ServiceDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["SERVICE_DSC"]);
        ...
        public ucLineServiceCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void CreateStructure()
		{
			...
			c.DataMember = "SERVICE_CD";
			c.ActAsSelector = true;
			c.UseHeaderSelector = false;
			c.Caption = null;
			c.Width = 30;

            c = DropDownList.Columns.Add("SERVICE_CD");
            c.DataMember = "SERVICE_CD";
            c.Caption = "Code";
            c.Width = 100;

			c = DropDownList.Columns.Add("SERVICE_DSC");
			c.DataMember = "SERVICE_DSC";
			c.Caption = "Service";
			c.Width = 300;
		...
		private void SetTableSource()
		{
            ...
            manager_actual_vs_budget mgr_avb = new manager_actual_vs_budget();
            ...
            sql_line_service ls = new sql_line_service();
            ls.Run();

            DataTable dt = ls.Data;
			ClsConnection.AddColumns(dt, "SERVICE_CD", "SERVICE_DSC");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more Services are invalid");
							break;
						}
					}
				}
