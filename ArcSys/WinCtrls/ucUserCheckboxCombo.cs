...
{
	/// <summary>ArcSys version of ucCheckedComboBox for user logins</summary>
	public partial class ucUserCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked user login names</summary>
		[Browsable(false)]
			...
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["User_Login"]);
		...
		/// <summary>Gets a comma separated string of user login names</summary>
		[Browsable(false)]
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more users are invalid");
							break;
						}
					}
				}
