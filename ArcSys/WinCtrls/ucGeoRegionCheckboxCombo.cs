...
{
	/// <summary>ArcSys version of ucCheckedComboBox for geo regions</summary>
	public partial class ucGeoRegionCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked geo region codes</summary>
		[Browsable(false)]
		public List<string> CheckedGeoRegionCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Geo_Region_Cd"]);
		...
		/// <summary>Gets a comma separated string of geo region codes</summary>
		[Browsable(false)]
		public string GeoRegionCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of geo region descriptions</summary>
		[Browsable(false)]
		public string GeoRegionDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Geo_Region_Dsc"]);
		...
		public ucGeoRegionCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void CreateStructure()
		{
			...
			c = DropDownList.Columns.Add("Geo_Region_Cd");
			c.DataMember = "Geo_Region_Cd";
			c.Caption = "Code";
			c.Width = 100;
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Geo_Region_Cd", "Geo_Region_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more regions are invalid");
							break;
						}
					}
				}
