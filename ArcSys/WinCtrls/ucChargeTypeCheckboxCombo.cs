...
{
	/// <summary>ArcSys version of ucCheckedComboBox for charge types</summary>
	public partial class ucChargeTypeCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked charge type codes</summary>
		[Browsable(false)]
		public List<string> CheckedChargeTypeCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Charge_Type_Cd"]);
		...
		/// <summary>Gets a comma separated string of charge type codes</summary>
		[Browsable(false)]
		public string ChargeTypeCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of charge type descriptions</summary>
		[Browsable(false)]
		public string ChargeTypeDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Charge_Type_Dsc"]);
		...
		public ucChargeTypeCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void CreateStructure()
		{
			...
			c = DropDownList.Columns.Add("Charge_Type_Cd");
			c.DataMember = "Charge_Type_Cd";
			c.Caption = "Code";
			c.Width = 100;
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Charge_Type_Cd", "Charge_Type_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more charge types are invalid");
							break;
						}
					}
				}
