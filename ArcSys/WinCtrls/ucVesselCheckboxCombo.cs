...
namespace CS2010.ArcSys.WinCtrls
{
	...
	public partial class ucVesselCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked vessel codes</summary>
		[Browsable(false)]
		public List<string> CheckedVesselCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Vessel_Cd"]);
			...
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Vessel_Nm"]);
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Vessel_Cd", "Vessel_Nm");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more vessels are invalid");
							break;
						}
					}
				}
