...
{
	/// <summary>ArcSys version of ucCheckedComboBox for vessels</summary>
	public partial class ucVoyageCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked vessel codes</summary>
		[Browsable(false)]
		public List<string> CheckedVoyageCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Voyage_Cd"]);
		...
		/// <summary>Gets a comma separated string of vessel codes</summary>
		[Browsable(false)]
		public string VoyageCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of vessel descriptions</summary>
		[Browsable(false)]
		public string VoyageDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Voyage_Cd"]);
		...
		private void CreateStructure()
		{
			...
			c.DataMember = "Voyage_Cd";
			c.ActAsSelector = true;
			c.UseHeaderSelector = false;
			c.Caption = null;
			c.Width = 30;

			c = DropDownList.Columns.Add("Voyage_Cd");
			c.DataMember = "Voyage_Cd";
			c.Caption = "Code";
			c.Width = 100;

			c = DropDownList.Columns.Add("Vessel_Cd");
		...
		private void SetTableSource()
		{
			...
			DataTable dt = ClsVVoyage.GetCurrentVoyages();
			ClsConnection.AddColumns(dt, "Voyage_Cd", "Vessel_Cd");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more voyages are invalid");
							break;
						}
					}
				}
