...
{
	/// <summary>ArcSys version of ucCheckedComboBox for charge categories</summary>
	public partial class ucChargeCategoryCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked charge category codes</summary>
		[Browsable(false)]
		public List<string> CheckedChargeCategoryCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Charge_Category_Cd"]);
		...
		/// <summary>Gets a comma separated string of charge category codes</summary>
		[Browsable(false)]
		public string ChargeCategoryCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of charge category descriptions</summary>
		[Browsable(false)]
		public string ChargeCategoryDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Charge_Category_Dsc"]);
		...
		public ucChargeCategoryCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Charge_Category_Cd", "Charge_Category_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more charge categories are invalid");
							break;
						}
					}
				}
