...
{
	/// <summary>ArcSys version of ucCheckedComboBox for vendors</summary>
	public partial class ucVendorCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked vendor codes</summary>
		[Browsable(false)]
		public List<string> CheckedVendorCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Vendor_Cd"]);
		...
		/// <summary>Gets a comma separated string of vendor codes</summary>
		[Browsable(false)]
		public string VendorCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of vendor names</summary>
		[Browsable(false)]
		public string VendorDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Vendor_Nm"]);
		...
		private void CreateStructure()
		{
			...
			c = DropDownList.Columns.Add("Vendor_Cd");
			c.DataMember = "Vendor_Cd";
			c.Caption = "Code";
			c.Width = 100;
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Vendor_Cd", "Vendor_Nm");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more vendors are invalid");
							break;
						}
					}
				}
