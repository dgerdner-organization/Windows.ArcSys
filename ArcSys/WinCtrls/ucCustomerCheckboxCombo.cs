...
{
	/// <summary>ArcSys version of ucCheckedComboBox for customers</summary>
	public partial class ucCustomerCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked customer codes</summary>
		[Browsable(false)]
		public List<string> CheckedCustomerCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Customer_Cd"]);
		...
		/// <summary>Gets a comma separated string of customer codes</summary>
		[Browsable(false)]
		public string CustomerCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of customer names</summary>
		[Browsable(false)]
		public string CustomerDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Customer_Nm"]);
		...
		public ucCustomerCheckBoxCombo()
		{
			InitializeComponent();
		...
		private void CreateStructure()
		{
			...
			c = DropDownList.Columns.Add("Customer_Cd");
			c.DataMember = "Customer_Cd";
			c.Caption = "Code";
			c.Width = 100;
		...
		private void SetTableSource()
		{
			...
			ClsConnection.AddColumns(dt, "Customer_Cd", "Customer_Nm");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more customers are invalid");
							break;
						}
					}
				}
