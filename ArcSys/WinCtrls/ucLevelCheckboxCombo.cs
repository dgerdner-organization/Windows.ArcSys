...
{
	/// <summary>ArcSys version of ucCheckedComboBox for charge levels</summary>
	public partial class ucLevelCheckBoxCombo : ucCheckedComboBox
	{
		#region Properties

		/// <summary>Gets a list of checked charge level codes</summary>
		[Browsable(false)]
		public List<string> CheckedLevelCDs
		{
			get
			{
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string cd = ClsConvert.ToString(drv["Level_Cd"]);
		...
		/// <summary>Gets a comma separated string of charge level codes</summary>
		[Browsable(false)]
		public string LevelCDs
		{
			get
			{
		...
		/// <summary>Gets a comma separated string of charge level descriptions</summary>
		[Browsable(false)]
		public string LevelDSCs
		{
			get
			{
				StringBuilder sb = new StringBuilder();
				...
				if( CheckedItems != null )
				{
					foreach( DataRowView drv in CheckedItems )
					{
						...
						string dsc = ClsConvert.ToString(drv["Level_Dsc"]);
		...
		private void CreateStructure()
		{
			...
			c = DropDownList.Columns.Add("Level_Dsc");
			c.DataMember = "Level_Dsc";
		...
		private void SetTableSource()
		{
			...
			DataTable dt = ClsLevel.GetAll();
			ClsConnection.AddColumns(dt, "Level_Cd", "Level_Dsc");
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more charge levels are invalid");
							break;
						}
					}
				}
