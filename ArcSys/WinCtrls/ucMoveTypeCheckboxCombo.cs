...
{
	/// <summary>ArcSys version of ucCheckedComboBox for a subset of ACMS move types (F1 thru F4
	/// are not needed and are excluded from the drop down)</summary>
	public partial class ucMoveTypeCheckBoxCombo : ucCheckedComboBox
	{
		#region Fields
		...
		/// <summary>Gets a comma separated string of move type codes</summary>
		[Browsable(false)]
		...
		public ucMoveTypeCheckBoxCombo()
		{
			InitializeComponent();
		...
		public void SetTableSource(bool bDoorOnly)
		{
			try
			{
				...
				if (bDoorOnly)
				{
					ComboItem[] lst = new ComboItem[5];
					lst[0] = new ComboItem("F5", "Door In / Free Out");
					lst[1] = new ComboItem("F6", "Door In / Liner Out");
					lst[2] = new ComboItem("F7", "Free In / Door Out");
					lst[3] = new ComboItem("F8", "Liner In / Door Out");
					lst[4] = new ComboItem("F9", "Door In / Door Out");
					base.DropDownDataSource = lst;
				}
				else
				{
					ComboItem[] lst = new ComboItem[9];
					lst[0] = new ComboItem("F1", "F1");
					lst[1] = new ComboItem("F2", "F2");
					lst[2] = new ComboItem("F3", "F3");
					lst[3] = new ComboItem("F4", "F4");
					lst[4] = new ComboItem("F5", "F5");
					lst[5] = new ComboItem("F6", "F6");
					lst[6] = new ComboItem("F7", "F7");
					lst[7] = new ComboItem("F8", "F8");
					lst[8] = new ComboItem("F9", "F9");
					base.DropDownDataSource = lst;
			...
			try
			{
				if( CheckedItems != null )
				{
					foreach( object obj in CheckedItems )
					{
						...
						{
							e.Cancel = true;
							Display.ShowError(ClsConfig.MessageBoxTitle, "One or more items are invalid");
							break;
						}
					}
				}
