...
namespace CS2010.ACMS.Business
{
	public partial class ClsBooking : ClsBaseTable
	{
		...
		public const string SelectSQL = @"SELECT * FROM T_BOOKING 
				LEFT OUTER JOIN R_LOCATION_TERMINAL
				ON T_BOOKING.POE_TERMINAL_CD=R_LOCATION_TERMINAL.TERMINAL_CD
				LEFT OUTER JOIN R_LOCATION_TERMINAL r_loc2
				ON T_BOOKING.POD_TERMINAL_CD=r_loc2.TERMINAL_CD ";
		...
		public const int Vessel_CdMax = 20;
		...
		public const int Location_Plor_CdMax = 10;
		public const int Location_Plod_CdMax = 10;
		...
		protected string _Location_Plor_Cd;
		protected string _Location_Plod_Cd;
		...
		public string Location_Plor_Cd
		{
			get { return _Location_Plor_Cd; }
			set
			{
				string val = ( string.IsNullOrEmpty(value) == false )
					? value.Trim() : null;
				...
				if( string.Compare(_Location_Plor_Cd, val, false) == 0 ) return;
		
				if( val != null && val.Length > Location_Plor_CdMax )
					_Location_Plor_Cd = val.Substring(0, (int)Location_Plor_CdMax);
				else
					_Location_Plor_Cd = val;
		
				_IsDirty = true;
				NotifyPropertyChanged("Location_Plor_Cd");
			}
		}
		public string Location_Plod_Cd
		{
			get { return _Location_Plod_Cd; }
			set
			{
				string val = ( string.IsNullOrEmpty(value) == false )
					? value.Trim() : null;
				...
				if( string.Compare(_Location_Plod_Cd, val, false) == 0 ) return;
		
				if( val != null && val.Length > Location_Plod_CdMax )
					_Location_Plod_Cd = val.Substring(0, (int)Location_Plod_CdMax);
				else
					_Location_Plod_Cd = val;
		
				_IsDirty = true;
				NotifyPropertyChanged("Location_Plod_Cd");
			}
		}
		...
		protected ClsLocationTerminal _Poe_Terminal;
		protected ClsLocationTerminal _Pod_Terminal;

		#endregion	// #region Foreign Key Fields

		#region Foreign Key Properties

		public ClsLocationTerminal Poe_Terminal
		{
			get
			{
				if( Poe_Terminal_Cd == null )
					_Poe_Terminal = null;
				else if( _Poe_Terminal == null ||
					_Poe_Terminal.Terminal_Cd != Poe_Terminal_Cd )
					_Poe_Terminal = ClsLocationTerminal.GetUsingKey(Poe_Terminal_Cd);
				return _Poe_Terminal;
			}
			set
			{
				if( _Poe_Terminal == value ) return;
		
				_Poe_Terminal = value;
			}
		}
		public ClsLocationTerminal Pod_Terminal
		{
			get
			{
				if( Pod_Terminal_Cd == null )
					_Pod_Terminal = null;
				else if( _Pod_Terminal == null ||
					_Pod_Terminal.Terminal_Cd != Pod_Terminal_Cd )
					_Pod_Terminal = ClsLocationTerminal.GetUsingKey(Pod_Terminal_Cd);
				return _Pod_Terminal;
			}
			set
			{
				if( _Pod_Terminal == value ) return;
		...
		public override void ResetColumns()
		{
			...
			Adj_Rdd_Dt = null;
			Adj_Rdd_Reason = null;
			Location_Plor_Cd = null;
			Location_Plod_Cd = null;
			Sea_Air_Flg = null;
		}
		...
		public void CopyFrom(ClsBooking src)
		{
			...
			Location_Plor_Cd = src._Location_Plor_Cd;
			Location_Plod_Cd = src._Location_Plod_Cd;
			Sea_Air_Flg = src._Sea_Air_Flg;
		}

		public override bool ReloadFromDB()
		{
			ClsBooking tmp = ClsBooking.GetUsingKey(Partner_Cd, Partner_Request_Cd);
		...
		public override int Insert()
		{
			...
			p[55] = Connection.GetParameter
				("@LOCATION_PLOR_CD", Location_Plor_Cd);
			p[56] = Connection.GetParameter
				("@LOCATION_PLOD_CD", Location_Plod_Cd);
			...
			const string sql = @"
				INSERT INTO T_BOOKING
					...
					ADJ_RDD_REASON, LOCATION_PLOR_CD, LOCATION_PLOD_CD,
					SEA_AIR_FLG)
				VALUES
					(@PARTNER_CD, @PARTNER_REQUEST_CD, @BOOKING_ID,
					@PROJECT_CD, @CONTRACT_NO, @TAC_NO,
		...
		public override void LoadFromDataReader(DbDataReader dr)
		{
			...
			Location_Plor_Cd = ClsConvert.ToString(dr["LOCATION_PLOR_CD"]);
			Location_Plod_Cd = ClsConvert.ToString(dr["LOCATION_PLOD_CD"]);
		...
		public override void LoadFromDataRow(DataRow dr)
		{
			...
			Location_Plor_Cd = ClsConvert.ToString(dr["LOCATION_PLOR_CD", v]);
			Location_Plod_Cd = ClsConvert.ToString(dr["LOCATION_PLOD_CD", v]);
		...
		public override void CopyToDataRow(DataRow dr)
		{
			...
			dr["LOCATION_PLOR_CD"] = ClsConvert.ToDbObject(Location_Plor_Cd);
			dr["LOCATION_PLOD_CD"] = ClsConvert.ToDbObject(Location_Plod_Cd);
		...
		public static ClsBooking GetUsingKey(string Partner_Cd, string Partner_Request_Cd)
		{
		...
		public static DataTable GetAll(string Poe_Terminal_Cd, string Pod_Terminal_Cd)
		{
			List<DbParameter> lst = new List<DbParameter>();	
			StringBuilder sb = new StringBuilder();
		
			if( string.IsNullOrEmpty(Poe_Terminal_Cd) == false )
			{
				lst.Add(Connection.GetParameter("@POE_TERMINAL_CD", Poe_Terminal_Cd));
				sb.Append(" WHERE T_BOOKING.POE_TERMINAL_CD=@POE_TERMINAL_CD");
			}
			if( string.IsNullOrEmpty(Pod_Terminal_Cd) == false )
			{
				lst.Add(Connection.GetParameter("@POD_TERMINAL_CD", Pod_Terminal_Cd));
				sb.AppendFormat(" {0} T_BOOKING.POD_TERMINAL_CD=@POD_TERMINAL_CD",
