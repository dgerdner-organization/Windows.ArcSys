...
{
	public partial class ClsLocationType : ClsBaseTable
	{
		...
		public const string TableName = "R_LOCATION_TYPE";
		...
		{
			return new string[PrimaryKeyCount] { "LOCATION_TYPE_CD" };
		}

		public const string SelectSQL = "SELECT * FROM R_LOCATION_TYPE";
		...
		public const int Location_Type_DscMax = 50;
		...
		protected string _Location_Type_Dsc;
		...
		public string Location_Type_Dsc
		{
			get { return _Location_Type_Dsc; }
			set
			{
				string val = ( string.IsNullOrEmpty(value) == false )
					? value.Trim() : null;
				...
				if( string.Compare(_Location_Type_Dsc, val, false) == 0 ) return;
		
				if( val != null && val.Length > Location_Type_DscMax )
					_Location_Type_Dsc = val.Substring(0, (int)Location_Type_DscMax);
				else
					_Location_Type_Dsc = val;
		
				_IsDirty = true;
				NotifyPropertyChanged("Location_Type_Dsc");
			}
		}
		...
		public ClsLocationType() : base() {}
		public ClsLocationType(DataRow dr) : base(dr) {}
		public ClsLocationType(ClsLocationType src) { CopyFrom(src); }
		...
		public void CopyFrom(ClsLocationType src)
		{
			Location_Type_Cd = src._Location_Type_Cd;
			Location_Type_Dsc = src._Location_Type_Dsc;
		...
		{
			ClsLocationType tmp = ClsLocationType.GetUsingKey(Location_Type_Cd);
		...
		public override int Insert()
		{
			...
			p[1] = Connection.GetParameter
				("@LOCATION_TYPE_DSC", Location_Type_Dsc);
			...
			const string sql = @"
				INSERT INTO R_LOCATION_TYPE
					(LOCATION_TYPE_CD, LOCATION_TYPE_DSC, ACTIVE_FLG)
				VALUES
					(@LOCATION_TYPE_CD, @LOCATION_TYPE_DSC, @ACTIVE_FLG)
				RETURNING
		...
		public override int Delete()
		{
			...
			const string sql = @"
				DELETE FROM R_LOCATION_TYPE WHERE
				LOCATION_TYPE_CD=@LOCATION_TYPE_CD";
		...
		public override void LoadFromDataReader(DbDataReader dr)
		{
			...
			Location_Type_Dsc = ClsConvert.ToString(dr["LOCATION_TYPE_DSC"]);
		...
		public override void LoadFromDataRow(DataRow dr)
		{
			...
			Location_Type_Dsc = ClsConvert.ToString(dr["LOCATION_TYPE_DSC", v]);
		...
		public override void CopyToDataRow(DataRow dr)
		{
			...
			dr["LOCATION_TYPE_DSC"] = ClsConvert.ToDbObject(Location_Type_Dsc);
		...
		public static ClsLocationType GetUsingKey(string Location_Type_Cd)
		{
			object[] vals = new object[] {Location_Type_Cd};
			DataRow dr = Connection.GetDataRowUsingKey
				(TableName, GetPrimaryKeys(), vals);
			return ( dr == null ) ? null : new ClsLocationType(dr);
		}
