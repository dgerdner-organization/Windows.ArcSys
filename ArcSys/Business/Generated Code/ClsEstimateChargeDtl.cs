...
namespace CS2010.ArcSys.Business
{
	public partial class ClsEstimateChargeDtl : ClsBaseTable
	{
		...
		public const string TableName = "T_ESTIMATE_CHARGE_DTL";
		...
		{
			return new string[PrimaryKeyCount] { "ESTIMATE_CHARGE_DTL_ID" };
		}

		public const string SelectSQL = @"SELECT * FROM T_ESTIMATE_CHARGE_DTL 
				LEFT OUTER JOIN R_MOVE_TYPE
				ON T_ESTIMATE_CHARGE_DTL.MOVE_TYPE_CD=R_MOVE_TYPE.MOVE_TYPE_CD
				LEFT OUTER JOIN R_LOCATION
				ON T_ESTIMATE_CHARGE_DTL.PLOD_LOCATION_CD=R_LOCATION.LOCATION_CD
				LEFT OUTER JOIN R_LOCATION r_loc2
				ON T_ESTIMATE_CHARGE_DTL.POD_LOCATION_CD=r_loc2.LOCATION_CD
				LEFT OUTER JOIN R_LOCATION r_loc3
				ON T_ESTIMATE_CHARGE_DTL.POL_LOCATION_CD=r_loc3.LOCATION_CD
				LEFT OUTER JOIN R_LOCATION r_loc4
				ON T_ESTIMATE_CHARGE_DTL.PLOR_LOCATION_CD=r_loc4.LOCATION_CD
				INNER JOIN T_CARGO_ACTIVITY
				ON T_ESTIMATE_CHARGE_DTL.CARGO_ACTIVITY_ID=T_CARGO_ACTIVITY.CARGO_ACTIVITY_ID
				INNER JOIN T_ESTIMATE_CHARGE
				ON T_ESTIMATE_CHARGE_DTL.ESTIMATE_CHARGE_ID=T_ESTIMATE_CHARGE.ESTIMATE_CHARGE_ID
				LEFT OUTER JOIN R_VESSEL
				ON T_ESTIMATE_CHARGE_DTL.VESSEL_CD=R_VESSEL.VESSEL_CD ";
		...
		protected Int64? _Estimate_Charge_Dtl_ID;
		...
		public Int64? Estimate_Charge_Dtl_ID
		{
			get { return _Estimate_Charge_Dtl_ID; }
			set
			{
				if( _Estimate_Charge_Dtl_ID == value ) return;
		
				_Estimate_Charge_Dtl_ID = value;
				_IsDirty = true;
				NotifyPropertyChanged("Estimate_Charge_Dtl_ID");
			}
		}
		...
		public ClsEstimateChargeDtl() : base() {}
		public ClsEstimateChargeDtl(DataRow dr) : base(dr) {}
		public ClsEstimateChargeDtl(ClsEstimateChargeDtl src) { CopyFrom(src); }
		...
		public void CopyFrom(ClsEstimateChargeDtl src)
		{
			Estimate_Charge_Dtl_ID = src._Estimate_Charge_Dtl_ID;
		...
		{
			ClsEstimateChargeDtl tmp = ClsEstimateChargeDtl.GetUsingKey(Estimate_Charge_Dtl_ID.Value);
		...
		public override int Insert()
		{
			...
			p[24] = Connection.GetParameter
				("@ESTIMATE_CHARGE_DTL_ID", Estimate_Charge_Dtl_ID, ParameterDirection.Output, DbType.Int64, 0);
			...
			const string sql = @"
				INSERT INTO T_ESTIMATE_CHARGE_DTL
					(ESTIMATE_CHARGE_DTL_ID, ESTIMATE_CHARGE_ID, CARGO_ACTIVITY_ID,
					...
					M_TONS)
				VALUES
					(ESTIMATE_CHARGE_DTL_ID_SEQ.NEXTVAL, @ESTIMATE_CHARGE_ID, @CARGO_ACTIVITY_ID,
					@ACTIVITY_AMT, @ACTIVITY_UNIT_QTY, @BOOKING_NO,
					@CUSTOMER_REF, @VESSEL_CD, @VOYAGE_NO,
					@SAIL_DT, @PLOR_LOCATION_CD, @POL_LOCATION_CD,
					@POD_LOCATION_CD, @PLOD_LOCATION_CD, @SERIAL_NO,
					...
					@M_TONS)
				RETURNING
					ESTIMATE_CHARGE_DTL_ID, CREATE_USER, CREATE_DT,
			...
			Estimate_Charge_Dtl_ID = ClsConvert.ToInt64Nullable(p[24].Value);
		...
		public override int Update()
		{
			...
			p[24] = Connection.GetParameter
				("@M_TONS", M_Tons);
			p[25] = Connection.GetParameter
				("@ESTIMATE_CHARGE_DTL_ID", Estimate_Charge_Dtl_ID);
			p[26] = Connection.GetParameter
				("@MODIFY_USER", Modify_User, ParameterDirection.Output, DbType.AnsiStringFixedLength, Modify_UserMax);

			const string sql = @"
				UPDATE T_ESTIMATE_CHARGE_DTL SET
					MODIFY_DT = @MODIFY_DT,
					ESTIMATE_CHARGE_ID = @ESTIMATE_CHARGE_ID,
					CARGO_ACTIVITY_ID = @CARGO_ACTIVITY_ID,
					ACTIVITY_AMT = @ACTIVITY_AMT,
		...
		public override int Delete()
		{
			...
			const string sql = @"
				DELETE FROM T_ESTIMATE_CHARGE_DTL WHERE
				ESTIMATE_CHARGE_DTL_ID=@ESTIMATE_CHARGE_DTL_ID";
		...
		public override void LoadFromDataReader(DbDataReader dr)
		{
			...
			Estimate_Charge_Dtl_ID = ClsConvert.ToInt64Nullable(dr["ESTIMATE_CHARGE_DTL_ID"]);
		...
		public override void LoadFromDataRow(DataRow dr)
		{
			...
			Estimate_Charge_Dtl_ID = ClsConvert.ToInt64Nullable(dr["ESTIMATE_CHARGE_DTL_ID", v]);
		...
		public override void CopyToDataRow(DataRow dr)
		{
			...
			dr["ESTIMATE_CHARGE_DTL_ID"] = ClsConvert.ToDbObject(Estimate_Charge_Dtl_ID);
		...
		public static ClsEstimateChargeDtl GetUsingKey(Int64 Estimate_Charge_Dtl_ID)
		{
			object[] vals = new object[] {Estimate_Charge_Dtl_ID};
			DataRow dr = Connection.GetDataRowUsingKey
				(TableName, GetPrimaryKeys(), vals);
			return ( dr == null ) ? null : new ClsEstimateChargeDtl(dr);
		}
		public static DataTable GetAll(string Move_Type_Cd, string Plod_Location_Cd,
			string Pod_Location_Cd, string Pol_Location_Cd,
			string Plor_Location_Cd, Int64? Cargo_Activity_ID,
			Int64? Estimate_Charge_ID, string Vessel_Cd)
		{
			...
			if( string.IsNullOrEmpty(Move_Type_Cd) == false )
			{
				...
				sb.Append(" WHERE T_ESTIMATE_CHARGE_DTL.MOVE_TYPE_CD=@MOVE_TYPE_CD");
			}
			if( string.IsNullOrEmpty(Plod_Location_Cd) == false )
			{
				lst.Add(Connection.GetParameter("@PLOD_LOCATION_CD", Plod_Location_Cd));
				sb.AppendFormat(" {0} T_ESTIMATE_CHARGE_DTL.PLOD_LOCATION_CD=@PLOD_LOCATION_CD",
